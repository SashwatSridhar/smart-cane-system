// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from smart_cane_interfaces:msg/UltrasonicData.idl
// generated code does not contain a copyright notice
#include "smart_cane_interfaces/msg/detail/ultrasonic_data__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"


// Include directives for member types
// Member `header`
#include "std_msgs/msg/detail/header__functions.h"

bool
smart_cane_interfaces__msg__UltrasonicData__init(smart_cane_interfaces__msg__UltrasonicData * msg)
{
  if (!msg) {
    return false;
  }
  // header
  if (!std_msgs__msg__Header__init(&msg->header)) {
    smart_cane_interfaces__msg__UltrasonicData__fini(msg);
    return false;
  }
  // distance_cm
  // confidence
  // obstacle_detected
  return true;
}

void
smart_cane_interfaces__msg__UltrasonicData__fini(smart_cane_interfaces__msg__UltrasonicData * msg)
{
  if (!msg) {
    return;
  }
  // header
  std_msgs__msg__Header__fini(&msg->header);
  // distance_cm
  // confidence
  // obstacle_detected
}

bool
smart_cane_interfaces__msg__UltrasonicData__are_equal(const smart_cane_interfaces__msg__UltrasonicData * lhs, const smart_cane_interfaces__msg__UltrasonicData * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // header
  if (!std_msgs__msg__Header__are_equal(
      &(lhs->header), &(rhs->header)))
  {
    return false;
  }
  // distance_cm
  if (lhs->distance_cm != rhs->distance_cm) {
    return false;
  }
  // confidence
  if (lhs->confidence != rhs->confidence) {
    return false;
  }
  // obstacle_detected
  if (lhs->obstacle_detected != rhs->obstacle_detected) {
    return false;
  }
  return true;
}

bool
smart_cane_interfaces__msg__UltrasonicData__copy(
  const smart_cane_interfaces__msg__UltrasonicData * input,
  smart_cane_interfaces__msg__UltrasonicData * output)
{
  if (!input || !output) {
    return false;
  }
  // header
  if (!std_msgs__msg__Header__copy(
      &(input->header), &(output->header)))
  {
    return false;
  }
  // distance_cm
  output->distance_cm = input->distance_cm;
  // confidence
  output->confidence = input->confidence;
  // obstacle_detected
  output->obstacle_detected = input->obstacle_detected;
  return true;
}

smart_cane_interfaces__msg__UltrasonicData *
smart_cane_interfaces__msg__UltrasonicData__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  smart_cane_interfaces__msg__UltrasonicData * msg = (smart_cane_interfaces__msg__UltrasonicData *)allocator.allocate(sizeof(smart_cane_interfaces__msg__UltrasonicData), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(smart_cane_interfaces__msg__UltrasonicData));
  bool success = smart_cane_interfaces__msg__UltrasonicData__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
smart_cane_interfaces__msg__UltrasonicData__destroy(smart_cane_interfaces__msg__UltrasonicData * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    smart_cane_interfaces__msg__UltrasonicData__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
smart_cane_interfaces__msg__UltrasonicData__Sequence__init(smart_cane_interfaces__msg__UltrasonicData__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  smart_cane_interfaces__msg__UltrasonicData * data = NULL;

  if (size) {
    data = (smart_cane_interfaces__msg__UltrasonicData *)allocator.zero_allocate(size, sizeof(smart_cane_interfaces__msg__UltrasonicData), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = smart_cane_interfaces__msg__UltrasonicData__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        smart_cane_interfaces__msg__UltrasonicData__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
smart_cane_interfaces__msg__UltrasonicData__Sequence__fini(smart_cane_interfaces__msg__UltrasonicData__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      smart_cane_interfaces__msg__UltrasonicData__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

smart_cane_interfaces__msg__UltrasonicData__Sequence *
smart_cane_interfaces__msg__UltrasonicData__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  smart_cane_interfaces__msg__UltrasonicData__Sequence * array = (smart_cane_interfaces__msg__UltrasonicData__Sequence *)allocator.allocate(sizeof(smart_cane_interfaces__msg__UltrasonicData__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = smart_cane_interfaces__msg__UltrasonicData__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
smart_cane_interfaces__msg__UltrasonicData__Sequence__destroy(smart_cane_interfaces__msg__UltrasonicData__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    smart_cane_interfaces__msg__UltrasonicData__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
smart_cane_interfaces__msg__UltrasonicData__Sequence__are_equal(const smart_cane_interfaces__msg__UltrasonicData__Sequence * lhs, const smart_cane_interfaces__msg__UltrasonicData__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!smart_cane_interfaces__msg__UltrasonicData__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
smart_cane_interfaces__msg__UltrasonicData__Sequence__copy(
  const smart_cane_interfaces__msg__UltrasonicData__Sequence * input,
  smart_cane_interfaces__msg__UltrasonicData__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(smart_cane_interfaces__msg__UltrasonicData);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    smart_cane_interfaces__msg__UltrasonicData * data =
      (smart_cane_interfaces__msg__UltrasonicData *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!smart_cane_interfaces__msg__UltrasonicData__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          smart_cane_interfaces__msg__UltrasonicData__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!smart_cane_interfaces__msg__UltrasonicData__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
