...
class ObstacleFusionNode(Node):
    ...
    def find_center_detection(self):
        ...
        if center_detections:
            best_center = max(center_detections, key=lambda x: x.results[0].score)
            return best_center
        ...
        if all_valid_detections:
            return max(all_valid_detections, key=lambda x: x.results[0].score)
    ...
    def process_fusion_timer(self):
        ...
        confidence = center_detection.results[0].score
        class_id = center_detection.results[0].id
